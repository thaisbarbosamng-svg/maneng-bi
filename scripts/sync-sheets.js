...
async function sincronizarSheetsParaFirestore(sheets) {
  ...
  console.log("  Carregando registros existentes...");
  ...
  for (const row of dados) {
    ...
    const sigla = get(C.SIGLA), cliente = get(C.CLIENTE), mes = get(C.MES), ano = get(C.ANO);
    ...
    const novosDados = {
      ...
      tipoManutencao: get(C.TIPO), status: get(C.STATUS).toUpperCase().trim() || "PROGRAMADA",
  ...
  for (let i = 0; i < ops.length; i += LOTE) {
    ...
    ops.slice(i, i + LOTE).forEach(op => op.type === "update" ? batch.update(op.ref, op.data) : batch.set(op.ref, op.data));
    await batch.commit();
...
async function sincronizarFirestoreParaSheets(sheets) {
  ...
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `'${ABA_BASE}'!A:Z` });
  const linhas = res.data.values || [];
  const headerIdx = linhas.findIndex(r => r.some(c => c.toString().trim().length > 0));
  const header = linhas[headerIdx].map(h => h.toString().trim().toUpperCase());
  const C = {
    ...
    STATUS: header.indexOf("STATUS"), DATA_INICIO: header.indexOf("DATA DE INICIO"),
  ...
  snap.forEach(docSnap => {
    ...
    for (let i = headerIdx + 1; i < linhas.length; i++) {
      const row = linhas[i];
      if ((row[C.SIGLA]||"").trim()===d.sigla && (row[C.CLIENTE]||"").trim()===d.cliente &&
          (row[C.MES]||"").trim()===d.mes && String(row[C.ANO]||"").trim()===String(d.ano)) {
        const r = i + 1;
        if (C.STATUS>=0&&d.status) updates.push({ range:`'${ABA_BASE}'!${colLetra(C.STATUS)}${r}`, values:[[d.status]] });
        if (C.DATA_INICIO>=0&&d.dataInicio) updates.push({ range:`'${ABA_BASE}'!${colLetra(C.DATA_INICIO)}${r}`, values:[[fmtDataBR(d.dataInicio)]] });
        if (C.DATA_TERMINO>=0&&d.dataTermino) updates.push({ range:`'${ABA_BASE}'!${colLetra(C.DATA_TERMINO)}${r}`, values:[[fmtDataBR(d.dataTermino)]] });
        if (C.PERIODICIDADE>=0&&d.periodicidade) updates.push({ range:`'${ABA_BASE}'!${colLetra(C.PERIODICIDADE)}${r}`, values:[[d.periodicidade]] });
        break;
      }
    }
...
function parseData(str) {
  ...
  try {
    ...
    let partes = str.includes("/") ? str.split("/").reverse() : str.split("-");
