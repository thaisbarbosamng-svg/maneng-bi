...
async function main() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ ultimaSync: new Date().toISOString(), ano: ANO_ATUAL, total: registros.length, registros })
  );
...
async function lerSheets(sheets) {
  ...
  console.log(`  Registros ${ANO_ATUAL}: ${dados.length}`);
